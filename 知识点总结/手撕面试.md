...
- 思路就是， 预处理 行列是否有0， 赋值即可O（M+N）的额外空间
- O（1）的思路是，用matrix的第一行和第一列记录该行or该列是否有0，作为标志位。 需要特判本身行和列本身就有0的情况，1. 记录第一行和列本身就有0的标记 2. 判断其他位置 3其他置1 4.处理第一行列
...
- 哈希表 时间复杂度O(max(m, n))
- 得到短链表的长度，作为末尾相同比较的一段，将长链表的头移动，循环比较
...
- 模拟做法
- 递归做法：https://leetcode.cn/problems/reverse-linked-list/solutions/36710/dong-hua-yan-shi-206-fan-zhuan-lian-biao-by-user74/
...
- O（N）时间 + O（N）空间复杂度：用一个数组存储一下，判断回文数组
- O（N）时间 + O（1）空间复杂度：双指针
  - 找到前半部分的尾节点
  - 翻转后半部分链表
  - 判断是否回文
  - 恢复链表
...
- 哈希表， 遍历节点 判断是否已经访问过
- 快慢指针， 龟兔赛跑算法， 快指针走两步，慢指针走一步， 如果没有环， 快指针一定会提前终止到null，如果有环， 快指针会优先进入环，慢指针随后， 之后快指针一定会在某个时间追上慢指针 套圈
...
- 也是模拟就好了，进位， 最后一位如果是1的话， 再加上

[19. 删除链表的倒数第 N 个结点](https://leetcode.cn/problems/remove-nth-node-from-end-of-list/)

- 快慢指针，快指针先走N步， （如果到了null，说明删第一个就行）， 之后，同时走，直到fast的下一步为null，此时slow就是要删的前一个节点

[24. 两两交换链表中的节点](https://leetcode.cn/problems/swap-nodes-in-pairs/)

- pre和cur怎么到下一个，就看第一个的相对位置
...
- 优先队列 N x log（k），每次找logk次数找到最小节点，需要找N次，N是元素个数
...
- 定义LCA(R, P, Q)：如果左子树和右子树 中同时包含p和q，R就是最近公共祖先， LCA表示往上返回 是否找到， 如果都没有 就往上返回null
...
- ms 维护最小值的栈， stk 维护加入的所有元素
- push：stk直接push， ms判断比peek小才push
- pop：stk直接pop， ms判断stk pop出的数是否和peek相同，相同也pop
- getmin：ms.peek
- top：stk.peek
...
- nums数字栈 dict 字符栈
- 遇到数字 加nums， 遇到字符和[ 加入dict， 遇到]开始清算， 弹出倍数 和 dict里面[之前的一段，拼接再push进去

### 🌟[739. 每日温度](https://leetcode.cn/problems/daily-temperatures/)

- 经典单调栈， 每个元素只会入栈出战一次 ON做法
- 从前往后面里，栈里面存储的是 还没有找到下一个比他大的位置的元素， 一旦找到了 就弹出栈 并更新
- 从后往前的思路是，栈里存的是下一个的最大位置，如果已经有了 72 69 再到71 的时候，69一定不可能是前面的下一个位置，可以不用考虑
...
class Solution {
    public int largestRectangleArea(int[] hs) {
        // 枚举高度 hs[i]
        // 对于每一个 hs[i] 能娶到的最大宽度是多少？
        // 预处理 l 和 r数组， l[i] 表示i左边，最近一个比他小的位置
        // r[i] 表示i右边 最近一个比他小的位置，
...
- 排序 nlogn
- 小根堆， k个， 不断删除最小的，剩下的都是最大的， peek就是第k大的 java里默认就是小根 nlogk
...
对顶堆的做法， 小根堆维护前一部分， 大根堆维护后一部分

维护前一部分的最小值 和 后一部分的最大值，假设 M和N分别为size

如果 M == N，返回 A.peek + B.peek >> 1， 如果M ！=N代码保证 M=N+1， 返回A.peek即可
...
class Solution {
    public int maxProfit(int[] prices) {
        int mi = prices[0];
        int ans = 0;
        // 如果第i天卖出， 利润就是 当天股价 - 之前最小的股价
        // mi维护之前最小的， ans维护利润最大的
...
class Solution {
    public boolean canJump(int[] nums) {
        ...
        for(int i = 0; i < n; i ++) {
            if(i > pre) {
                return false; // 如果之前最远的一步都走不到这，就说明到不了
            }
            int far = nums[i] + i; // 更新一下最远的距离
...
```
f[i][j]表示  走到 i, j 位置上的 总共有多少条不同的路径， 答案返回 f[m - 1][n - 1]

怎么转移， 第一行 第一列的情况， 只能从单侧来， f = 1, 其他情况，有 从上和左两个位置来 
f[i][j] = f[i - 1][j] + f[i][j - 1]

初始化 第一行 第一列 都是1 

遍历顺序 i 和 j都依赖 上一步， 从左到右，从上到下
```
...
```
f[i][j] 表示 走到i，j位置上的 最小路径总和, 返回 f[m - 1][n - 1]

怎么转移 
- 对于第一行 f[i][j] = f[i][j - 1] + grid[i][j]
- 对于第一列 f[i][j] = f[i - 1][j] + grid[i][j]
- 其他的位置 f[i][j] = min(f[i - 1][j], f[i][j - 1]) + grid[i][j]
...
怎么转移, 思考，长度小的 转移到长度大的 len = j - i + 1;
...
if(len == 1)  f[i][j] 一定为true
if(len == 2 || len == 3) f[i][j] 的值取决于 s[i] == s[j]
if(len > 3) f[i][j] = f[i+1][j-1] && (s[i] == s[j])
```

- 中心扩散法
  - 遍历每一个点，从该点开始像两边扩散 
  - 分奇偶的情况 centerSpread 走两遍， (i, i) (i, i+1)
...
class Solution {
    public boolean wordBreak(String s, List<String> wordDict) {
        ...
        // 转移 f[i]
        // for x in worddict:
        // if s[i - x: i] in dict => f[i] = f[i - x.len]
        // 初始化
        ...
        // 状态转移 f[i - w.length()] 转移过来，
        // 条件是： s[i - w.length() + 1, i] 在set里面
        for(int i = 0; i < n; i++) {
            for(String w : wordDict) {
                if(i < w.length()) {
                    continue;
                }
                if(f[i - w.length()] && S.contains(s.substring(i - w.length() + 1, i + 1))) {
                    f[i] = true;
                    break;
                }
            }
...
class Solution {
    public int majorityElement(int[] nums) {
        int votes = 0;
        int ans = 0;
      // votes为0时 假设当前的数为众数， 开始计数， 是+1，不是-1
        for(int num : nums) {
            if(votes == 0) ans = num;
            votes += num == ans ? 1 : -1; 
        }
