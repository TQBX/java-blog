...
AOF，只追加文件，append-only file：每执行一条会更改Redis中的数据的命令，Redis就会将该命令写入硬盘中的AOF文件。实时性更好，已成为主流的持久化方案。默认情况下Redis没有开启，需要通过appendonly yes开启。

![image-20240726181921113](img/Redis%E7%9F%A5%E8%AF%86%E7%82%B9%E6%80%BB%E7%BB%93/image-20240726181921113.png)

为了兼顾数据和写入性能，可以采用第二个。

执行bgrewriteaof命令，让aof文件执行重写功能，用最少的命令达到相同的效果，减少aof文件大小。
