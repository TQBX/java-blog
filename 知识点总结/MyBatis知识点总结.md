...
1. 一级缓存：（session级别）在同一个 SqlSession 中执行相同的 SQL 语句时，查询的结果会被缓存起来，默认开启，无法关闭
2. 二级缓存：（namespace和mapper作用域）多个 SqlSession 共享同一个缓存，当多个 SqlSession 执行相同的 SQL 语句时，查询的结果会被缓存起来，需要手动开启（不默认开启就是为了考虑数据一致性和并发访问）：全剧配置：cacheEnabled=true， useCache=true
...
![image-20240727010116052](img/MyBatis%E7%9F%A5%E8%AF%86%E7%82%B9%E6%80%BB%E7%BB%93/image-20240727010116052.png)

# mybatis支持延迟加载吗？

![image-20240727010412098](img/MyBatis%E7%9F%A5%E8%AF%86%E7%82%B9%E6%80%BB%E7%BB%93/image-20240727010412098.png)

支持，但是默认没开启

fetchType = "lazy" or 配置文件中：lazyLoadingEnabled=true

延迟加载原理：

1. 使用cglib创建目标对象的代理对象
2. 当调用目标方法user.getOrderList()时，进入拦截器invoke方法，发现user.getOrderList()是null，执行sql查询order列表
3. 把order查询上来，然后调用`user.setOrderList(List<Order> orderList)`
